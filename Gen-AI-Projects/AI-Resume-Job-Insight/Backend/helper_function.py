...
def comparing(resume : dict,jobdes : dict)-> str:
    response_schema = [
        ResponseSchema(name="Match Percentage", description="Carefully and strictly analyze Percentage of match between resume and job description this is mandatory thing and important feature correctly analyze it(these feature id for Match Percentage that shows how similar the overall requirements are matched between both)."),
...
def create_resume_pdf(resume_data: dict, file_name: str = "resume.pdf") -> tuple[bool, str]:
    """
    Create a professional PDF resume matching the RAZA-RESUME.pdf theme with black-only colors
    ...
    try:
        ...
        # Create PDF object with custom page setup
        pdf = FPDF()
        pdf.add_page()
        ...
        # ============ HEADER SECTION ============
        # Use real personal information from parsed data (with fallback extraction)
        def extract_personal_info(resume_data):
            # First priority: Use directly parsed personal info
            personal_info = {
            ...
            # If we have the main fields, return them
        ...
        print(f"🔍 Backend Personal Info Extracted: {personal_info}")
        ...
        # Professional Title extracted from actual work experience
        ...
        elif "Data Analyst" in work_exp or "Data Expert" in work_exp:
            title = "DATA ANALYST"
        elif "Developer" in work_exp:
            title = "SOFTWARE DEVELOPER"
        elif "Engineer" in work_exp:
            title = "SOFTWARE ENGINEER"
        
        pdf.set_font('Arial', '', 12)  # Reduced from 14
        pdf.set_text_color(0, 0, 0)  # Black
        pdf.cell(0, 8, title, ln=True, align='C')
        pdf.ln(3)
        
        # Build real contact info from extracted data
        contact_parts = []
        ...
        contact_info = clean_unicode_text(contact_info)
        ...
        if resume_data.get('Summary'):
            ...
            pdf.set_font('Arial', '', 10)  # Reduced from 11
            pdf.set_text_color(0, 0, 0)  # Black
            summary = clean_unicode_text(resume_data.get('Summary'))
            ...
            for word in words:
                test_line = line + word + " "
                if pdf.get_string_width(test_line) < 150:  # Max width
                    line = test_line
                else:
                    if line:
        ...
        if resume_data.get('Education'):
            ...
            # Format education with simple bullet points
            ...
            for line in education_lines:
                if line.strip():
                    ...
                    while remaining_text:
                        if pdf.get_string_width(remaining_text) < 145:
                            pdf.cell(0, 5, remaining_text, ln=True)
                            break
                        else:
                            ...
                            for i, word in enumerate(words):
                                test_text = line_text + word + " "
                                if pdf.get_string_width(test_text) < 145:
                                    line_text = test_text
                                else:
                                    break
                            
                            if line_text:
                                pdf.cell(0, 5, line_text.strip(), ln=True)
                                remaining_text = remaining_text[len(line_text):].strip()
                                pdf.cell(5, 5, '', ln=False)  # Indent continuation
                            else:
                                # Force break if word is too long
                                pdf.cell(0, 5, remaining_text[:50] + "...", ln=True)
                                break
            pdf.ln(3)
        ...
        if resume_data.get('Work Experience'):
            ...
            # Format work experience with proper structure
            exp_lines = work_exp.split('\n') if '\n' in work_exp else [work_exp]
            for line in exp_lines:
                if line.strip():
                    # Check if it's a position/company line
                    if any(keyword in line.lower() for keyword in ['engineer', 'developer', 'analyst', 'manager', 'intern', 'designer', '20']):
        ...
        if resume_data.get('Projects'):
            ...
            for line in project_lines:
                if line.strip():
                    # Check if it's a project title (usually in caps or contains certain keywords)
                    if (line.isupper() or 
                        any(keyword in line.lower() for keyword in ['project', 'system', 'application', 'website', 'app', 'dashboard'])):
                        pdf.set_font('Arial', 'B', 10)
                        pdf.cell(0, 5, line.strip(), ln=True)
                        pdf.set_font('Arial', '', 10)
                    else:
        ...
        if resume_data.get('Additional Info'):
            ...
            pdf.set_font('Arial', '', 10)
            pdf.set_text_color(0, 0, 0)  # Black
            additional = clean_unicode_text(resume_data.get('Additional Info'))
            
            add_lines = additional.split('\n') if '\n' in additional else [additional]
        ...
        # ============ FOOTER ============
        # Only add footer if there's space, otherwise skip to avoid blank pages
        current_y = pdf.get_y()
        if current_y < 270:  # If there's enough space on current page
            pdf.ln(5)
            pdf.set_font('Arial', 'I', 8)
            pdf.set_text_color(128, 128, 128)  # Light gray
            pdf.cell(0, 5, f'Generated on: {datetime.datetime.now().strftime("%B %d, %Y")}', ln=True, align='C')
        
        # Convert to base64 with comprehensive Unicode handling
        try:
            # Get PDF content and clean any remaining Unicode
            ...
            if isinstance(pdf_content, bytes):
                ...
                # Clean and encode string content
                pdf_content = clean_unicode_text(pdf_content)
                pdf_bytes = pdf_content.encode('latin1', errors='replace')
                pdf_base64 = base64.b64encode(pdf_bytes).decode('utf-8')
            
            print(f"✅ Professional PDF created successfully. Size: {len(pdf_content if isinstance(pdf_content, bytes) else pdf_bytes)} bytes")
            ...
            try:
                ...
                if isinstance(pdf_content, str):
                    pdf_content = pdf_content.encode('ascii', errors='replace').decode('ascii')
                    pdf_bytes = pdf_content.encode('latin1', errors='replace')
                else:
                    pdf_bytes = pdf_content
                
                pdf_base64 = base64.b64encode(pdf_bytes).decode('utf-8')
                print("✅ PDF created with ASCII fallback encoding")
                return True, pdf_base64
                
            except Exception as final_error:
                print(f"❌ Final encoding attempt failed: {final_error}")
                return False, f"PDF encoding failed: {str(final_error)}"
