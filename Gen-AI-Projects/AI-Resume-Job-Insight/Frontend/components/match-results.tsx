...
export function MatchResults({ result, onReset }: MatchResultsProps) {
  ...
  const parseSkills = (skillsInput: string | string[] | undefined | null): string[] => {
    ...
    // If it's a string, check if it's a JSON array first
    if (typeof skillsInput === 'string') {
      // Handle JSON array strings like "[\"Go\", \"Node.js\", \"RESTful API design\"]"
      if (skillsInput.trim().startsWith('[') && skillsInput.trim().endsWith(']')) {
        try {
          ...
          console.log('🔧 Failed to parse skills as JSON array, falling back to string parsing');
        }
      }
      
      // Try to extract skills from text patterns
      let cleanedInput = skillsInput;
      
      // Remove common prefixes
      cleanedInput = cleanedInput.replace(/^(Missing|Weak|Required)?\s*(Skills?|Technologies?|Tools?)?\s*:?\s*/i, '');
      ...
      // Standard comma/semicolon separation
      return cleanedInput
  ...
  // Debug missing skills data (keeping minimal logging)
  console.log('🔍 Missing Skills Debug:');
  console.log('- analysis.missingWeakSkills:', result.analysis.missingWeakSkills);
  console.log('- compareResponse.missingWeakSkills:', result.compareResponse.missingWeakSkills);
  
  const rawMissingSkills = result.analysis.missingWeakSkills || result.compareResponse.missingWeakSkills;
  console.log('- Using missing skills:', rawMissingSkills);
  
  const missingSkills = parseSkills(rawMissingSkills);
  
  console.log('- Final parsed array:', missingSkills);
  console.log('- Array length:', missingSkills.length);

  // Parse experience values with multiple fallbacks
  const candidateExp = parseFloat(
    (result.analysis as any)["visual Candidate Experience (years)"] ||
  ...
  let requiredExp = parseFloat(
    (result.analysis as any)["visual Required Experience (years)"] ||
              ...
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <div className="text-xs bg-gray-100 p-3 rounded">
                    ...
                    <p><strong>Visual Candidate Experience:</strong> {(result.analysis as any)["visual Candidate Experience (years)"] || 'Not found'}</p>
                    <p><strong>Visual Required Experience:</strong> {(result.analysis as any)["visual Required Experience (years)"] || 'Not found'}</p>
